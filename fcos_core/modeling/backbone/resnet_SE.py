...
class SpatialAttention(nn.Module):
    def __init__(self, kernel_size=7):
        ...
        self.conv1 = nn.Conv2d(3, 64, kernel_size, padding=padding, bias=False)
...
class Bottleneck(nn.Module):
    def __init__(
        ...
        self.se = ChannelGate(out_channels, 16, ['avg','max'])
    ...
    def forward(self, x):
        ...
        out2 = (torch.sum(out[0], dim=0)).data.cpu().numpy()
        out2 = 255 - (out2-out2.min())/(out2.max()-out2.min())
        out2 *= 255
...
_STAGE_SPECS = Registry({
    ...
    "R-50-FPN-CBAM": ResNet50FPNStagesTo5,
